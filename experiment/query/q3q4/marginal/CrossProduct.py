...
def evaluate_CP(inputfile, num_DO, attr1,attr2,e):
    ...
    print('Runtime(AS_Cros_Product2) = ', end_P22 - start_P22)
    print('Runtime(AS_Projection) = ', end_P23 - start_P23)
    print('Runtime(CSP_generate_labEncProduct22) = ', end_P24 - start_P24)
    
    N=[100,250,1500,1000,2000,3256]
    for j in range(6):
        num_DO=N[j]
        
        start_P25 = time.time()
        C1=As.generateEncCount_GroupBy(C,num_DO)
        end_P25 = time.time()
        
        start_P26 = time.time()
        C2=As.Laplace_vector(e,C1)
        end_P26 = time.time()
        ...
        print('The round of data owner =', num_DO)
        print("========")
        print('Runtime(AS_generateEncCount_GroupBy) = ', end_P25 - start_P25)
        print('Runtime(AS_Laplace_vectorLHE) = ', end_P26 - start_P26)
        print('Runtime(CSP_Lap_mul_decrypt) = ', end_P27 - start_P27)
