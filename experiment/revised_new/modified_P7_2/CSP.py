...
class CSProvider:
    ...
    def Lap_vec(self,prod,e):
        ...
        size1=len(prod)
        for i in range(size1):
           a_tilda=self.msk.labDecrypt(prod[i])
           u=np.random.geometric(1-pow(2.713,-e))
           v=np.random.geometric(1-pow(2.713,-e))
           z=u-v
           a_tilda+=z
    ...
    def generate_ciphers(self):
        self.cipher1=[0 for x in range(101)]
        self.cipher0=[0 for x in range(3256100)]
        t=0
        for i in range(100):
            self.cipher1[i]=self.mpk.labEncrypt(self.seed,1,1,self.enc_seed)
        for i in range(3256100):
            
            self.cipher0[i]=self.mpk.labEncrypt(self.seed,1,0,self.enc_seed)
            s=time.time()
            #print(i)
            p=time.time()
            t=t+p-s
        return t
    def generate_ciphers2(self):
        self.cipher1=[0 for x in range(101)]
        self.cipher0=[0 for x in range(3256100)]
        t=0
        c1=self.mpk.labEncrypt(self.seed,1,1,self.enc_seed)
        c2=self.mpk.labEncrypt(self.seed,1,0,self.enc_seed)
        for i in range(100):
            self.cipher1[i]=c1
        for i in range(3256100):
            ...
            t=t+p-s
        return t
    def generateEncCoding2(self,cnt,k):
        """
        Generates encrypted 1-hot-coding for the given counts
        """
        a=0
        ...
        for i in range(n):
            for j in range(k):
               ...
               if(j+1==cnt[i]):
                  code[i].append(self.cipher1[a])
                  a=a+1
               else:
                  code[i].append(self.cipher0[b])
                  b=b+1
    ...
    def decrypt1hotcoding(self,enc_coding,r_start,r_end):
          ...
          return v  
    def generate_labEncProduct22(self,prod,pk1,pk2,numDO):
        labEnc_prod=[[] for x  in range(len(prod))]
        for i in range(numDO):
            sigma1=self.msk.decrypt(pk1[i][0])
            #sigma2=self.msk.decrypt(pk2[i][0])
            b1=space_mapping(sigma1,self.mpk.n)
            #b2=space_mapping(sigma2,self.mpk.n)
            hh=space_mapping(b1*b1,self.mpk.n)
            for j in range(len(prod[0])):
                a_tilda=self.msk.decrypt(prod[i][j])
                a_tilda+=hh
                label=1
