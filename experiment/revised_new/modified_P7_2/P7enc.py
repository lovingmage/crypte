...
def evaluate_GroupBy(inputfile, num_DO, Q):
  ...
  for i in range(6):
   ...
   for j in range(5):
    ...
    if(flag==1):
      as4_s=time.time()
      enc_count=As.threshold_GroupBy(enc_coding,r_end,r_start) 
      as4_e=time.time()
      as5_s=time.time()
    ...
    gt_end = time.time()             
    gt_time_list[j]=(gt_end - gt_start)
   ...
   print("Mean and Std are=", gt_time_list.mean(), gt_time_list.std())
