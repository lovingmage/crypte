...
class AnalyticsServer:
    ...
    def ProjectionT(self,db,attribute,record_size):
        """
        Perform projection to retain only one-hot-codings for the attributes in 'attribute'
        Param:db- Input database
              record_size- number of attributes per record
   ...
   #def noisy_rangetree(self,attr,e):
    ...
    def generic_count(self, table):
        ...
        return v
    def generate_testvector(self,numDO):
        c=self.mpk.labEncrypt(self.seed,1,1,self.enc_seed)
        vec=[]
        ...
        for i in range(100):
            for j in range(numDO):
                table[i].append(c)
        print("Length of",len(table[0]),len(table))
        return table
    ...
    def Laplace_vector(self,e,C):
        ...
        for i in range(n):
              ...
              C[i]+=enc_z
        return C
    def Laplace_vectorLHE(self,e,C):
        n=len(C)
        for i in range(n):
              u=np.random.geometric(1-pow(2.713,-e))
              v=np.random.geometric(1-pow(2.713,-e))
              z=u-v
              enc_z=self.mpk.encrypt(z)
              C[i]+=enc_z
        return C
    ...
    def count_distinct(self,table):
         ...
         for i in range(self.num_DO):
             for j in range(n):
                 v[j]+=table[i][j]
         return v
    def demasked(self,table):

         n=len(table)
         for i in range(n):
                 table[i].hm-=self.mask[i]
         return table

    def CrossProduct2(self,attr1,attr2):
        """
        Generate cross product of two attributes from oroginal data
        """
        size1=self.domain_size[attr1]
        size2=self.domain_size[attr2]
        pk=[0 for x in range(self.num_DO)]
        newTable=[]
        d_size=[]
        for i in range(self.l):
             if(i!=attr1 and i!=attr2):
                d_size.append(self.domain_size[i])
        d_size.append(size1*size2)
        self.domain_size=d_size
        pk1=[[] for x in range(self.num_DO)]
        pk2=[[] for x in range(self.num_DO)]
        for i in range(self.num_DO):
            code=[]
            v1=self.merged_enc_X[i*self.l+attr1]
            v2=self.merged_enc_X[i*self.l+attr2]
            for t in range(self.l):
                if(t!=attr1 and t!=attr2):
                    newTable.append(self.merged_enc_X[i*self.l+t])
            pk.append(v1[0].sigma)
            for j in range(size1):
                for k in range(size2):
                    m=mul(v1[j],v2[k])
                    pk1[i].append(v1[j].enc_mask)
                    pk2[i].append(v2[k].enc_mask)
                    code.append(m)
            newTable.append(code)
        return(pk1,pk2,newTable)
