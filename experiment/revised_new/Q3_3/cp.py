...
def evaluate_CP(inputfile, num_DO, attr1,attr2,e):
    ...
    for i in range(6):
     ...
     time_set = np.zeros(5)
     for j in range(5):  
      inner_lp_start = time.time()
      #AS gen Groupby
      start_P25 = time.time()
      C1=As.generateEncCount_GroupBy(A,num_DO)
      end_P25 = time.time()
      
      #AS add noise
      start_P26 = time.time()
      C2=As.Laplace_vector(e/2,C1)
      end_P26 = time.time()
    
      #CSP add noise
      start_P27 = time.time()
      C3=csp.Lap_vec(C1,e/2) 
      end_P27 = time.time()
    
      #print (C3)   
      sum_AsEncCount=sum_AsEncCount+end_P25-start_P25
      sum_AsLap=sum_AsLap+end_P26-start_P26
      sum_cspLap=sum_cspLap+end_P27-start_P27
      inner_lp_end = time.time()
     
      time_set[j] = inner_lp_end - inner_lp_start
     
     print("NUM_DO",num_DO)
     print("sum_AsEncCount",sum_AsEncCount/float(10))
     print("sum_AsLap",sum_AsLap/float(10))
     print("sum_cspLap",sum_cspLap/float(10))  
     print("Mean and Std of running time")
     print(time_set.mean(), time_set.std())
    #l1_err = 0
    ...
    for i in range(10):
        ...
        for k in range(reps):
            C1=As.Laplace_vector(e/2,C)
            C2=csp.Lap_vec(C1,num_DO,e/2)
            l1_err = 0
