...
def evaluate_cp2_2(inputfile, num_DO,attr1,attr2, Q,e):
    """
    compute 
    ...
    print('Runtime(AS_Projection_Filter) = ', end_P22 - start_P22)
    print('Runtime(gen_bit_table) = ', end_P23 - start_P23)
    print('Runtime(As_CrossProduct2) = ', end_P24 - start_P24)
    print('Runtime(As_ProjectionT) = ', end_P24_2 - start_P24_2)
    print('Runtime(CSP_generate_labEncProduct22) = ', end_P25 - start_P25)
    ...
    for i in range(2):
        num_DO=N[i]
        As.set_numDO(num_DO)
        time_slot = np.zeros(5)
        time_break = np.zeros(4)
        for j in range(5):
            ...
            t3 = time.time()
            C1=As.Laplace_vector(e,enc_cnt)
            t4 = time.time()
            C2=csp.Lap_mul_decrypt2(C1,pk1,pk2,num_DO, e)
            t5 = time.time()
            end_inner = time.time()
            time_slot[j] = end_inner - start_inner
            time_break[0] += t2 - t1
            time_break[1] += t3 - t2
            time_break[2] += t4 - t3
            time_break[3] += t5 - t4
        ...
        print("Current Round is Size=", N[i])
        print("AS_filter_GroupBy=", time_break[0]/10)
        print("AS_generateEncCount_GroupBy=", time_break[1]/10)
        print("AS_Laplace_vector=", time_break[2]/10)
        print("Csp_Lap_mul_decrypt2=", time_break[3]/10)
        print(" ")
        print("The mean and std of running time: ")
        print(time_slot.mean(), time_slot.std())
    ...
    print('Runtime(AS_MergeEncX) = ', end_P21 - start_P21)
    print('Runtime(AS_Projection_Filter) = ', end_P22 - start_P22)
    print('Runtime(bitTable_Gen) = ', end_P23 - start_P23)
    print('Runtime(AS_CrossProduct2_ProjectionT) = ', end_P24 - start_P24)
    print('Runtime(CSP_generate_labEncProduct22) = ', end_P25 - start_P25)
    print('Runtime(AS_generateEncCount_GroupBy) = ', end_P26 - start_P26)
    print('Runtime(AS_Laplace_vectorLHE) = ', end_P27 - start_P27)
    print('Runtime(CSP_Lap_mul_decrypt2) = ', end_P28 - start_P28)
    '''
    

    #cnt is the input to the countDistinct SMC    
    #The following is the exp dispatcher
    ...
    for i in range(10):
        ...
        T2 = []
        for j in range(reps):
            T1=As.Laplace_vectorLHE(e, enc_cnt)
            T2=csp.Lap_mul_decrypt2(T1, pk1, pk2, num_DO, e)
            tmpErr = 0
            for k in range(len(T2)):
                tmpErr += abs(T2[k] - TC2[k])
            err_list[j] = tmpErr
