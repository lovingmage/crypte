...
def error_metric():
    ...
    #abs_error = np.abs(REAL_DIST[599] - ESTIMATE_DIST[599]) 
    #print(ESTIMATE_DIST)
...
# This is for the noise cdf
'''
def error_metric():
    cdf = lp.generate_cdf(REAL_DIST)
    ncdf = lp.generate_cdf(ESTIMATE_DIST, mode="ldp")
    cdf = cdf/cdf[-1]
    ncdf = ncdf/ncdf[-1]
    
    print(ncdf)
    
    return np.sum(abs(cdf-ncdf)
'''
...
def dispatcher():
    global g
    for e in np.arange(1.1, 1.3, 0.2):
    #for e in [0.1, 1, 5, 10, 30]:
        start_P1 = time.time()
